...
vec2 :: proc(v: ^rl.Vector2, min: f32 = -INF, max: f32 = INF, step: f32 = 0.1, label: cstring = nil) {
    vec_rect(flex_rect(), v, min, max, step, label)
    if true do return
    rect := flex_rect()
    rect.width /= 2
...
vec3 :: proc(v: ^rl.Vector3, min: f32 = -INF, max: f32 = INF, step: f32 = 0.1, label: cstring = nil) {
    // rect := flex_rect()
    // rect.width /= 3
...
vec_rect :: proc(rect: rl.Rectangle, v: ^[$N]f32, min: f32 = -INF, max: f32 = INF, step: f32 = 0.1, label: cstring = nil) {
    rect := rect
    rect.width /= N
    float_rect(rect, &v[0], min, max, step, label)

    for i := 1; i < N; i += 1 {
        label: cstring = " " if label != nil else nil // empty label to pad height.
        rect.x += rect.width
        float_rect(rect, &v[i], min, max, step, label)

        // Divider separating elements.
        divider := rect
        if label != nil {
            _, divider = label_split_rect(rect, label)
        }
        div_padding := divider.height / 4
        rl.DrawLineEx({divider.x, divider.y + div_padding}, {divider.x, divider.y + divider.height - div_padding}, 1, TEXT_COLOR)
    }
